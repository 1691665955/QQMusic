...
class MusicLyricVC: UIViewController,UITableViewDelegate,UITableViewDataSource,MZMusicPlayerManagerDelegate,UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.titleLB.setBackgroundColr(backgroundColr: UIColor.clear, text: self.musicItem.songname, font: UIFont.systemFont(ofSize: 18), textColor: UIColor.white,textAlignment:NSTextAlignment.center)
        ...
        self.authorLB.text = self.musicItem.singername
        ...
        let imgData = MusicDatabaseManager.share.queryLargeAlbumImage(songid: musicItem.songid)
        if imgData != nil {
            self.albumImageView.image = UIImage.init(data: imgData!)
            self.bgView.image = UIImage.init(data: imgData!)
        } else {
            self.albumImageView.sd_setImage(with: URL.init(string: musicItem.albumpic_big!), placeholderImage: UIImage.init(named: "QQListBack"))
            self.bgView.sd_setImage(with: URL.init(string: musicItem.albumpic_big!), placeholderImage: UIImage.init(named: "QQListBack"))
        }
    ...
    func playMusic(musicItem: MusicItem) {
        ...
        self.titleLB.setBackgroundColr(backgroundColr: UIColor.clear, text: musicItem.songname, font: UIFont.systemFont(ofSize: 20), textColor: UIColor.white,textAlignment:NSTextAlignment.center)
